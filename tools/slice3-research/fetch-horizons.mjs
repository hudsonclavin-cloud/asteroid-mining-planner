...
const BODIES = [
  ...
  { name: 'io', command: '501', center: '500@599' },
  { name: 'europa', command: '502', center: '500@599' },
  { name: 'ganymede', command: '503', center: '500@599' },
  { name: 'callisto', command: '504', center: '500@599' },
];
...
async function fetchDataset(body, cadence) {
  const outputPath = path.join(windowDataDir, `${cadence.label}-${body.name}.json`);
