// B1 — Slice 3 INV-009 Cutover Validation Harness
// Validates per-body Hermite interpolation error against INV-009 cutover bars,
// verifies HELIO <-> JUPITER frame round-trip bounds, and ensures the runtime
...
const tempOutDir = path.join(repoRoot, '.tmp-tests', 'v2-slice3-cutover');
...
console.log('Compiling v2 core and boundary for Slice 3 cutover...');
...
for (const [bodyId, { truthFile, barKm }] of Object.entries(BODY_CONFIG)) {
  ...
  console.log(`PASS INV-009 ${formatBodySummary(bodyId, summary)}`);
...
const ioTruthStates = loadTruthStates(core, 'io', BODY_CONFIG.io.truthFile, core.FRAME_JUPITER_J2000_ICRF);

core.configureFrameTransformHooks({
  jupiterHeliocentricStateProvider: buildInterpolatingProvider(jupiterSamples, core.interpolateBodyState),
});

let maxRoundTripError = 0;
let maxChainTenError = 0;

for (const truthState of ioTruthStates) {
  const tdbSeconds = truthState.tdbSeconds;
  if (tdbSeconds < ioSamples[0].tdbSeconds || tdbSeconds > ioSamples.at(-1).tdbSeconds) {
    continue;
  }

  const ioJupiter = core.interpolateBodyStateSeries('io', ioSamples, tdbSeconds);
  ...
  core.assertCanonicalState(ioHelio);
...
console.log('');
console.log('Slice 3 per-body interpolation summary:');
for (const summary of bodySummaries) {
  console.log(`  ${formatBodySummary(summary.bodyId, summary)}`);
}
console.log('');
console.log('Slice 3 frame round-trip summary:');
...
console.log('');
console.log('Slice 3 cutover harness passed with zero runtime invariant violations.');
