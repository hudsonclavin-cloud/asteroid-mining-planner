...
const tempOutDir = path.join(repoRoot, '.tmp-tests', 'v2-core-hermite-saturn');
...
test('Slice 4 Saturn-system Hermite interpolation stays below INV-010 bars', async () => {
  ...
  for (const [bodyId, { truthFile, barKm }] of Object.entries(BODY_CONFIG)) {
    ...
    console.log(
      `slice4Hermite ${bodyId} maxErrorKm=${maxErrorKm} rmsErrorKm=${rmsErrorKm} barKm=${barKm} checked=${errorsKm.length}`
    );
