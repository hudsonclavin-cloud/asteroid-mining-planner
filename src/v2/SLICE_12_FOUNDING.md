...
**CLOSURE CORRECTION (2026-07-02, Phase E audit finding M-C — appended, original text above preserved as the record of what was claimed):** The closure above overstated provenance in two ways. (1) It validated the single-rev `lambert()` (izzo, M=0) path — not the `lambertMultiRev` M=1 path the dedicated route ships — so "oracle-validated DLA" was scoped to M=0 over 2026-2032 only; Slice 11's M∈{1,2} validation compared scalar magnitudes only (c3/vInfDep/vInfArr at `tools/slice11-research/measurements/multi-rev-poliastro-validate.mjs:315-317`), leaving M=1 vector DIRECTIONS externally unvalidated — the exact INV-021 magnitude-vs-component gap. (2) "branch mismatches 0" quoted a dead counter: `branchMismatches` was declared and never incremented, and with single-branch solves on both sides it was structurally incapable of counting anything; the quote is retracted, and the counter is removed from the harness (schemaVersion 2). The extended harness (same file, default phase) now validates the shipped M=1 path directly: Apophis + Bennu 25×25 grids, **921 M=1-selected cells** compared against poliastro izzo M=1 (both branches; correspondence established per-cell by minimum vector distance — measured mapping left→highpath / right→lowpath, min discrimination margin ratio 1.58e14, zero ambiguous cells), **max vInfDep angular separation 5.74e-13 deg** (RMS 4.26e-14), max componentwise relative error 1.49e-14, **max |delta DLA| 5.61e-13 deg** (RMS 2.78e-14), zero solution-existence mismatches (329 cells below T_min on both sides). Bars: angular separation <= 1e-6 deg AND |delta DLA| <= 1e-6 deg — **PASS**. Data: `tools/slice12-research/data/dla-oracle-m1-vectors.json`, commit `830a4d9`. Method note recorded in the data file: the extension's first run produced a false FAIL from the acos angular-separation metric's double-precision floor (~1.2e-6 deg, sitting at the bar); the metric was corrected to the stable atan2(|a×b|, a·b) form with the bar unchanged, and both runs are documented in `metricNote`.
...
- **2026-07-02 (post-audit, oracle extension):** OQ-12-3 closure corrected (append-only) — M=1 vector-direction oracle extension PASS: 921 M=1-selected cells (Apophis + Bennu), max vInfDep angular separation 5.74e-13 deg, max |delta DLA| 5.61e-13 deg, branch mapping measured (left→highpath / right→lowpath, margin 1.58e14, zero ambiguous). Dead branchMismatch counter removed from the harness and its quote retracted. Data: `tools/slice12-research/data/dla-oracle-m1-vectors.json` (commit `830a4d9`). Closes audit finding M-C.
- **2026-07-02: SLICE 12 CLOSED.** Phases A-E complete, audit findings resolved (H-1 AMD-12-1, H-2 attribution, M's: disclosures, signed contours, M=1 vector oracle, doc errata; 8 LOWs parked). Deployed by this closeout build commit.
- *(Lock entry, remaining phase completions, and OQ closures to be appended.)*
- **2026-09-22 (docs correction, additive; no DEC/OQ/AMD changes).** The 2026-07-01 entry above calls 11.715 km/s the "max Earth |vZ| ... from the worker's own fixture". That is not a span maximum — it is the **four-epoch SAMPLE maximum** taken by the OQ-12-1 frame recon, which §4's OQ-12-1 resolution states correctly ("sampled Earth's velocity ... at four epochs across 2027: |vZ| reached **11.715 km/s** (2027-10-01)"). Re-measured across the whole fixture through the production ingest (`src/v2/boundary/horizons.ts` `ingestSlice2Fixture`; no rotation is applied to this fixture): the **span maximum is 11.89731636992642 km/s at JD TDB 2461120.5 (2026-03-21 TDB)**. The probe's own epoch reproduces exactly — 11.71467991580171 km/s at 2027-10-01 — and even calendar 2027's own maximum is 11.88634484921835 km/s (2027-03-21), so the four sampled epochs did not contain that year's peak either. Script and artifact, committed so the figure is reproducible: `tools/slice12-research/measure-earth-vz.mjs` -> `tools/slice12-research/data/earth-vz-span-max.json` (fixture `src/v2/data/horizons-inner-solar-system-2026-2040.json`, sha256 `c91f4e50da0e...`, 5,479 Earth samples, canonical frame `FRAME_HELIO_J2000_ICRF`). **DEC-12-2 is UNAFFECTED, and strengthened.** Its ruling — this fixture is ICRF/equatorial *by measurement*, and no rotation is applied — rests on |vZ| being large where an ecliptic frame would show ~0. A larger span maximum is stronger evidence, not weaker: measured |vZ|max / |v|max = 0.392650878065271 against sin(23.4392911 deg) = 0.3977771559319137, with a span minimum of 0.001467157129840613 km/s at a solstice (2034-06-22 TDB). The 11.715 figure stays correct as what it always was — a sample.
