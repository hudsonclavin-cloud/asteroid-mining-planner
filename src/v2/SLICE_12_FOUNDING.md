...
Candidates: (a) construct DLA in poliastro from the same states our solver uses and compare per-cell across a validation grid (extends the OQ-3 harness); (b) cross-check a handful of windows against the JPL Trajectory Browser's displayed DLA for a known target; (c) reproduce a published mission DLA (DART or OSIRIS-REx) at its actual launch date. Proposed bars, to be confirmed at closure: the frame rotation and arcsin are unit-tested at machine precision (~1e-14 — it is closed-form algebra); end-to-end agreement with an external tool at ≤ 0.5°, the looser bar justified by ephemeris and epoch-convention differences between tools (justification recorded per the audit-tolerance rule). Numbers not in our repo (e.g. a "published mission DLA ≈ X°" figure) must be looked up and cited at closure, not asserted from memory.
