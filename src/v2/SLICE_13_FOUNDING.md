...
**OQ-13-5 — Interpolation oracle: what validates the curves? STATUS: CLOSED 2026-07-03 — Phase B oracle (`808e709`) STRICT PASS: max relative error 1.18% (RMS 0.55%), New Glenn steep-segment optimism +3.11% measured and disclosed (DEC-13-7). See §8.**
...
- 2026-07-03 — **SLICE CLOSED.** DECs 13-1…13-9 delivered; OQ-13-1/2/3/4/5 all resolved; Phase F audit clean and remediated; deployed at `1accf9d`. Live: mission cost card with sourced vehicle curves, two-regime dogleg pricing, mission-mode toggle, delivered-mass headline; showcase body 2020 FK3.
- 2026-07-03 — **Post-close hygiene CLOSED. Parked: nothing.** Slice 13 fully closed — all post-close hygiene landed: New Glenn C3=5 anchor (`df3225f`), single-sourced constants (`6181bcc`), L9 trio (`fca3c6c`), and STATUS queued-dispatches section (`1f8c408`).
...
## Post-lock annotation (2026-08-01, additive) — showcase figures currently unreproducible from this repository

**Marker:** `S16-REMEDIATE-2026-08-01-A` · audit finding L3-6.

The OQ-13-3 resolution above reports measured recon figures — the 41,866-body scan, 2020 FK3's C3/DLA/convergence values, the RED-cell counts and rankings — citing `aster-audit-reports/slice13-showcase-recon.md`. **That report is not tracked in this repository, and no committed generator reproduces the full result.** These figures are **not asserted to be wrong** — independent spot-reads have reproduced adjacent public numbers — but as of this annotation they are **unreproducible from the repository alone**, which is below the evidence standard the project applies elsewhere (INV-034-class: evidence called upon must be recoverable). They should be treated as unverified until the scan is regenerated by a tracked, deterministic script whose artifact is committed and whose numbers are compared against every figure above. Until then, public surfaces should not cite them as validated.
