...
**OQ-2 CLOSED (2026-06-30) — 500-body measurement.** Extended Measurement 2 to 500 bodies (DEC-10), seed 11, stratified ATE/APO/AMO/IEO (actual counts 125/212/125/38 — IEO population-limited to 38). Result: meaningfulWinFraction = 0.242 (121/500), consistent with the 100-body estimate (0.28), confirming the win rate is NOT an artifact of small-sample variance. Orbit-class breakdown is the key finding: AMO 41.6% (52/125), IEO 26.3% (10/38), APO 23.6% (50/212), ATE 7.2% (9/125) — M=1 benefit is highly orbit-class-dependent, not uniform. Zero fully-failed bodies (no convergence breakdowns at population scale).

DECISION: a full-catalog M=1 re-screen + schemaVersion 2 + catalog UI refit (originally scoped as uniform Slice 11.5 work) is DEFERRED, not pursued as originally scoped. Rationale: the orbit-class split shows uniform re-screening would over-invest in low-benefit classes (Atens, 7%) to capture high-benefit classes (Amors, 42%). Future work, if pursued, should be orbit-class-targeted rather than a blanket schema/UI upgrade — scoped in its own founding doc against this data, not the original 28% single-number estimate. Phase E (this measurement) is the complete Slice 11.5 deliverable; schema/UI work is parked for a future slice.
