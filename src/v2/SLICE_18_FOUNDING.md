...
**2026-09-21 — Addendum to the close-out record: a self-caught correction to Items 5 and 6, same day, before merge.** Written while preparing the final report, from a check the second agent would have been asked to make. The L1 `encounter` landed at `5926b16` was the row `dv-scope-per-body.json` records — the LARGEST added-drift encounter, which is what makes a body material. DEC-18-7 bounds support at "a body's materially perturbing encounter", and the Front C design used the EARLIEST material one: the first moment the orbit is materially invalidated. Recomputed from the committed CAD snapshot with the DEC-18-6 criterion, **4,770 of the 10,150 L1 bodies have more than one individually material encounter row, and for 2,352 of them the earliest precedes the largest-drift row** — so the sentence "supported through [DATE]" and the W4/W5 boundary shipped at `5926b16` / `cecbc5b` over-claimed support for those 2,352 bodies. Corrected at `d496532`: `encounter` is now the earliest row after the body's element epoch whose own added drift reaches 10⁶ km; the largest-drift row's date is kept as `maxDriftEncounterCd` so the artifact stays traceable to `dv-scope-per-body.json`. `docs/` refreshed at `b6d3373` (only the tier artifact changed; every bundle name is unchanged from `4d3bae2`).

Consequences, stated plainly: for every affected body the boundary moved EARLIER, never later (0 moved later) — the correction is conservative for all 2,352 and changes no body's tier (populations recounted: 11 / 10,150 / 31,745 / 688 / 41,906; every non-encounter field byte-identical), so DEC-18-6's regression criterion (no over-prediction of materiality) is untouched. The flagship example changes: Apophis (99942) now reads "supported through 2028-09-12 … changes its orbit by an estimated 2.6 m/s" — its 2028-Sep-12 Earth pass at ~0.27 AU is individually material under the criterion (2.6 m/s carried to 2046 exceeds 10⁶ km) — where it previously read 2029-04-13 / 2,776 m/s; the 2029 flyby remains in the record as `maxDriftEncounterCd`. This is what the locked criterion says, applied strictly; C-5 already recorded that L1 is threshold-fragile, and the same fragility shows here as a date. **Ratification requested:** if Hudson prefers the largest-drift semantics, or a two-encounter sentence, it is a one-line change in `tier-sizing.mjs` and a regeneration — both encounters are already in the artifact, so no new data is needed either way. The artifact is now 11.01 MB.

The two verification claims in the entry above that this supersedes: "all 10,150 L1 encounters equal `dv-scope-per-body.json`" now holds for `maxDriftEncounterCd` (exact, all 10,150, a different script), while `encounter` is verified by an independent re-implementation of the criterion over the CAD snapshot inside `tests/v2-tier-disclosure.test.mjs`, which also pins the 2,352 count; and the Apophis boundary named in the Item 6 line (2029-04-13) is now 2028-09-12. Suite after the correction: 81 files / 303 tests / 0 failures; tsc clean.

**2026-09-21 — Follow-ups 1–4 before merge (Hudson's rulings of 2026-09-21, same branch).**

**(1) `tier-sizing-results.json` — real history, and the re-add reversed.** `git log --all -- tools/slice18-research/tier-sizing-results.json`: committed at `2eaf936` (2026-09-09, with this document); DELETED at `1cd5124` (2026-09-13, 20:30 −04:00, "remove superseded tier aggregate", which also removed the generator's aggregate write); re-added by the close-out run at `cba6e6a` and rewritten at `5926b16` / `d496532`; removed again at `0a8bbac`, and `tier-sizing.mjs` no longer writes it. Two corrections to the record: (a) the close-out entry above says the path "had never been git-tracked" — that was FALSE, inferred from absence at HEAD (`git ls-files`) instead of from history; the earlier report of a 2026-09-09 commit was correct. (b) C-4 and C-5 cite `tools/slice18-research/tier-sizing-results.json` as the stale artifact: C-5 was written at `ca2208d` (2026-09-13, 17:08 −04:00) while the file still existed, and `1cd5124` removed it 3.4 hours later — doing what C-5 asked ("regenerated or removed rather than left to be cited"). Read those citations as pointing to `tools/slice18-research/tier-sizing-per-body.json`, the ONE source of tier populations (its `populations` field) and of every per-body record. The aggregates the removed file carried are recountable from those records — `jupiterCrossingAlsoHasMaterialCadEncounter` = 9 is the count of `classification === 'jupiterCrossing' && material` — and the generator prints them to stdout only.

**(2) Boundary semantics RATIFIED: the earliest material encounter** — per DEC-18-3's annotation ("a body's first materially perturbing encounter") and because it can only under-claim. At `5ac5cb5` the largest-drift row is carried as `maxDriftEncounter` only when it is a different close approach from the boundary (2,352 of the 10,150 L1 bodies), and for those the L1 sentence appends, verbatim: "The largest perturbation in the window is a close approach to [BODY] on [DATE], changing its orbit by an estimated [VALUE] m/s." `maxDriftEncounter ?? encounter` equals `dv-scope-per-body.json`'s row for all 10,150 (0 mismatches); every other field byte-identical; populations unchanged (11 / 10,150 / 31,745 / 688 / 41,906); the artifact shrank 11.01 → 10.88 MB. Apophis now reads "supported through 2028-09-12 … 2.6 m/s … The largest perturbation in the window is a close approach to Earth on 2029-04-13, changing its orbit by an estimated 2,776 m/s."

Recorded: **the boundary date is model-derived and conservative by construction, not measurement-validated.** It comes from CAD `dist` and `v_rel`, the δv formula, and the 10⁶ km criterion, which never over-calls materiality (DEC-18-6: 0 over-predictions in 17 measured bodies). For 99942 the measurement in `nea-drift-results.json` shows drift staying near 44k km/yr until the 2029 flyby: `A_primaryAnchor.yearly_km` reads 47,885 km at +1 y and 91,427 km at +2 y from the 2026-05-01 anchor (JD 2461161.5), i.e. ~44–48k km/yr, and `closeApproaches[0].drift_before_km` = 33,477 km immediately before 2029-04-13 — nothing material shows up in the measurement across the 2028-09-12 pass. Its 2028-09-12 boundary is therefore conservative by roughly seven months (213 days to 2029-04-13).

**(3) Comets get their own sentence** (`052d0a2`). Structurally blind is split by cause, verified from the artifact: 206 `structurally-blind-comet` + 482 `structurally-blind-jupiter-crossing` = 688 = `populations.structurallyBlindL2`. Comets, verbatim: "Aster cannot bound the screening error for this object. It is a comet, which Aster's catalog classifies as not safe for two-body Keplerian propagation." — checked against the catalog fixture for all 206 (JFC 164 / HTC 36 / ETC 6; `inv014Tier` = `not-kepler-safe` for every one). Jupiter-crossing keeps the aphelion sentence, and all 482 have Q ≥ 4.95 AU (minimum 4.952), so it fits every one of them. This closes the 26-low-Q-comet caveat in the close-out entry above.

**(4) Tier-artifact load behaviour, reported: EAGER.** `src/v2/app/catalog-list/panel.ts` calls `loadTierAssignments()` at module load, so the solar-system page fetches the whole artifact (10,880,093 bytes at `5ac5cb5`) on every load before any tier badge can render; the dedicated porkchop page and the compare page also load it eagerly, in their initial `Promise.all`. Not changed here; carried as an open item in STATUS.

`docs/` rebuilt at `c6dc7dd`: entry bundles `porkchopV2-z3nhVK2W.js`, `solarSystemV2-Z_zaFg6C.js`, `compareV2-BbpSakkn.js`; `docs/_staging_v1/` byte-identical to HEAD by blob id. Suite after these follow-ups: 81 files / 304 pass / 0 fail / 1 skipped (one run was interrupted by the laptop's ~12-hour sleep — four `v2-render-*` files were killed by the 180 s watchdog on resume, wall clock 42,319 s — and re-run clean); tsc clean after every commit.

*Verification of this entry: the file's history was read with `git log --all`, not inferred from `git ls-files`; every count was recounted from the artifact at `5ac5cb5` (206 / 482 / 688 / 2,352 / 10,150) and cross-checked against the catalog fixture and `dv-scope-per-body.json`; the 99942 figures were read from the named fields of `nea-drift-results.json` and the 213 days computed from the two dates; bundle names were read from `docs/assets/` after the build. Additive-only: `git diff --cached -- src/v2/SLICE_18_FOUNDING.md | grep '^-'` yields the file header only.*
