...
| Local HEAD (branch `claude/s18-close-out`) | `3d395fb` | Slice 18 close-out run (`d8c4681` … `4d3bae2`, founding close record `4d3267c`, STATUS `63841cc`, L1-boundary correction `d496532` + `b6d3373` + `4c1c58d`, STATUS `836bf2c`) and the four pre-merge follow-ups (`0a8bbac` results-file re-add reversed, `5ac5cb5` boundary ratified + largest-perturbation sentence, `052d0a2` comet sentence, `c6dc7dd` docs, `3d395fb` founding entry); this STATUS commit lands on top of it |

**Measured pin lag (2026-09-21):** before this edit, `git rev-list --count 3d395fb..HEAD` measured 0 commits and `git rev-list --count 81252f2..HEAD` measured 19. After this STATUS commit lands, those measure 1 and 20. STATUS commits LAST in any sequence.
**Push state:** origin/main is at `81252f2`. Branch `claude/s18-close-out` holds twenty local-only commits and is NOT merged. No agent pushes, ever. The merge is Hudson's: `git checkout main; git merge claude/s18-close-out --ff-only; git hpush`.
...
**Deploy boundary:** live `docs/` is `81252f2` (rebuilt 2026-09-20; live bundles `porkchopV2-Ci6r-fmN.js`, `solarSystemV2-DFJ0f0Yh.js`, `compareV2-CEg78aFw.js`). The branch's rebuild is `c6dc7dd` (after follow-ups 1-3; supersedes `4d3bae2` / `b6d3373`) — live only once merged and pushed — with entry bundles `porkchopV2-z3nhVK2W.js`, `solarSystemV2-Z_zaFg6C.js`, `compareV2-BbpSakkn.js`, shared chunks `porkchop-view-BTebVubw.js`, `tier-assignments-C-5EqJQL.js`, `resolve-catalog-body-pmopDgKo.js`, `store-CkJiJhKO.js`, and the tier artifact `docs/tier-sizing-per-body.json` (10,880,093 bytes, sha256 `eb118425…`, identical to the `tools/` copy). Bundle names from the earlier branch rebuilds (`porkchopV2-DPwWKoPv.js`, `solarSystemV2-BM6ai_je.js`, `compareV2-Br2FOkFz.js`) were never live and are gone. The `5a00907` boundary previously recorded here was stale: `81252f2` had already rebuilt `docs/` on 2026-09-20.
...
### MATH-LAYER AUDIT + FRONT C SCOPE — 2026-09-09

**Math-layer audit: CLEAN on everything load-bearing.** All five functions the slice rests on — `propagateKeplerianStateVectors`, `cartesianToElements`, izzo `lambert`, `lambertMultiRev`, `interpolateBodyStateSeries` — plus constants, unit and frame conversions, and the derived product quantities were audited against closed forms, conserved quantities, round-trip identities, cross-solver comparison, and adversarial inputs. **No defect invalidates any committed measurement or any locked DEC.** Headline tolerances: energy and angular momentum conserved to ≤7.1e-14 and ≤3.7e-15 relative over 2026–2046 on real bodies; Kepler residual ≤1.8e-15 rad over 17,280 solves; Householder true residual ≤1.2e-15; the obliquity rotation verified applied exactly once, in the correct direction (i=0 orbit pole matches the independently computed rotated ecliptic pole to 1.2e-16); the two independent UTC→TDB implementations agree bit-identically. The Lambert BVP chain — Lambert v1 → `cartesianToElements` → propagate → must land on r2, three independent code paths that must all be right — closes with a **mean residual of 9 mm** over 1,022 AU-scale transfers. Artifacts: `Documents\aster-slice18\audit\` (not in version control).

**Cross-solver widened from 1 cell to 1,296 geometries** (transfer angle 1°–359° including 179.9°/181°, TOF 30–1800 d, z-offsets, r₂ 0.7–2.5 AU): izzo and `lambertMultiRev` are **bit-identical at M=0 — worst difference exactly 0 in both x and velocity** — and agree on solvability in 1,296 of 1,296 cases. The Front A "structural, not numerical" finding is confirmed on a wide domain, not a single cell.

**Oracle closure (2026-09-09).** The external validation is a committed poliastro dual-oracle run (`multi-rev-poliastro-validation.json`, M=0 max relative error 3.35e-14 over 2,500 cells). It binds today's HEAD only if the solver has not changed since. The **transitive import closure** of the two validated entry points was computed from source — **7 files, self-contained, zero external specifiers** — and every file diffed against `6d68b3b`, the solver commit the fixture itself names: **zero code changes; comments only.** `stumpff.ts` was in the prior hand-list but is NOT in the closure — it is dead code in v2. The earlier hand-listed check reached the right answer; this proves it rather than asserting it.

**F1 (LOW) — izzo `lambert()` accepts negative time-of-flight and returns `ok:true` with garbage.** `tof = −100 d` yields x = −1.743, |v1| = 62.4 km/s. Two gaps compound: no `tof > 0` guard (`lambertMultiRev` has one), and `tof.ts` `compute_psi` silently returns 0 for x < −1, outside the solver's domain, letting the iteration "converge" on a meaningless root. **Reachability: unreachable from every shipped path** — the grid's TOF axis starts at 182.5 d, the cache at 182 d, both linspace-positive, and the porkchop calls `lambertMultiRev`, which guards. No committed measurement used a non-positive TOF. **F2 (LOW) — izzo mislabels invalid input** (NaN, zero, ∞ TOF) as `no_convergence` rather than an invalid-input reason; never returns `ok:true`, so it is a taxonomy defect, not a correctness one. **Neither is fixed** — math-layer changes need a ruling and a multi-agent audit, and repairing mid-audit would destroy the evidence of what shipped.

**Latent hazards recorded, not defects:** classical-element ill-conditioning at i≈0 ∧ ν≈180° (round-trip up to 7.2 km, unacknowledged in code, eight orders below the drift floor and guarded by every measurement's per-body sanity gate); the `GM_SUN` comment says "IAU 2015 nominal" but carries the DE430 value — the DE441 mismatch behind the Horizons truth is 1.7e-10 relative, ≈15 km over 19 years, negligible, but the label is wrong; `cartesianToElements` passes NaN through silently; the colormap paints an `ok` cell with null C3 as bright-MAX; `console.debug` fires per no-solution cell.

**FRONT C SCOPE — the denominator inverts when computed with δv instead of distance.** The prior probe reported 2,644 bodies (6.3%) within 5M km and read as "a minority annotation." Recomputed on a wider CAD pull (dist-max 0.3 AU, `body=ALL`, 49,629 records) with the physical criterion — δv = 2·v·sin(atan(μ/(d·v²))), material when δv × (time remaining to 2046-01-01) reaches the regime where argmins were measured to move — **10,173 catalog bodies (24.3%) reach the ≥10⁶ km regime and 15,459 (36.9%) exceed the 10⁵ km planet floor.** Front C is not a minority annotation.

**That number is a LOWER BOUND, and the validation says so.** Against the 17 measured NEA-band bodies the criterion agrees on 7, **under-predicts 10, and over-predicts 0** — it never calls a body material that measurement says is not. Where an encounter drives the drift the magnitudes track: 2012 UE34 predicted 176.3M vs measured 164.4M km; 2025 HH 17.3M vs 18.4M. **5 of the 10 misses are Jupiter-crossing (Q from 5.2 to 705 AU)** whose Jupiter passes CAD cannot see; the other 5 are inner bodies drifting secularly with no encounter at all.

**Measured API bound, from the response not the documentation:** CAD stores non-Earth approaches only to **0.15 AU for Venus, 0.10 AU for Mercury, 0.25 AU for Mars, 0.28 AU for Jupiter** — a Venus query at dist-max 0.5 returns the same 9,442 rows with a maximum distance of exactly 0.1500 AU. Saturn returns zero rows in the window. Blind spot quantified: a Venus pass at the 0.15 AU cap yields δv ≈ 2.9 m/s → ~1.7M km over 19 years, and a Jupiter pass at its 0.28 AU cap yields ~358M km — both above the argmin-moving regime. **Encounters that matter exist beyond what the API will return.**

**δv is ADOPTED as Front C's instrument** in place of a tuned distance threshold: it is a computed physical quantity, its inputs (`dist`, `v_rel`, perturber GM) arrive in one API call, it reproduces the measured cliff, and it explains the nulls — a distant pass fails materiality because its δv falls below the body's secular drift, not because a threshold excluded it. Front C's disclosure DESIGN remains undecided; only the instrument and the scope are.
...
**Commits (all on the branch, in order):** `d8c4681` hyperbolic propagation guard — 2015 D1 no longer throws on the PC modal, the detailed route, or compare; 433 / 2018 LA byte-identical by sha256 probe · `a9ed4f0` `/v2/porkchop/?body=433` resolves like compare (shared `src/v2/boundary/resolve-catalog-body.ts`) · `c4da66f` L0 rows drop the LOW/HIGH C3 quality badge, keep C3 de-emphasised with "computed from the last known orbit" · `cba6e6a` `tier-sizing.mjs` field fix (9, not 0) and `tier-sizing-results.json` committed for the first time; populations recounted unchanged · `5926b16` full disclosure strings on the catalog tooltip, compare cell and porkchop page, plus the L0 / L1 / L2 / PROP FAIL legend; the artifact now carries the values (impactor termination dates, 3D first-day drift, L1 encounters) · `cecbc5b` per-cell L1 support boundary on the dedicated page (wash, dashed curve, readouts, extremes) · `34b707c` overlay modal M=0 default with an M=1 contour-layer toggle · `4d3bae2` docs rebuild · `4d3267c` founding-doc close record · `63841cc` first STATUS close · **`d496532` self-caught correction: the L1 support boundary is the EARLIEST material encounter** (the artifact had carried the largest-drift row; 2,352 of 10,150 L1 bodies were over-claimed — boundary moved earlier for each, never later; no tier changed; Apophis now reads 2028-09-12 / 2.6 m/s, the 2029-04-13 flyby kept as `maxDriftEncounterCd`) · `b6d3373` docs tier artifact refreshed · `4c1c58d` founding-doc addendum · `836bf2c` STATUS re-pin · **follow-ups (Hudson, 2026-09-21):** `0a8bbac` `tier-sizing-results.json` re-add REVERSED (real history: committed `2eaf936` 2026-09-09, deleted `1cd5124` 2026-09-13, re-added `cba6e6a` — the close-out claim "never git-tracked" was wrong; the per-body artifact is the one source; the generator no longer writes the aggregate) · `5ac5cb5` boundary RATIFIED as the earliest material encounter, `maxDriftEncounter` carried where it differs (2,352 bodies) and the L1 sentence appends "The largest perturbation in the window is a close approach to [BODY] on [DATE], changing its orbit by an estimated [VALUE] m/s." · `052d0a2` comets get their own sentence (206 + 482 = 688 verified from the artifact; all 206 not-kepler-safe in the catalog) · `c6dc7dd` docs rebuild · `3d395fb` founding-doc §8 entry.

**Revert map (each independent unless stated):** `d8c4681` guard; `a9ed4f0` resolver; `c4da66f` badge; `cba6e6a` generator + artifacts; `5926b16` disclosure strings + artifact fields — `cecbc5b` depends on it (it reads `encounter.jd`), so revert `cecbc5b` first or together; `34b707c` overlay; `4d3bae2` docs (rebuild after any other revert); `4d3267c` founding-doc entry (additive — reverting it is a deletion and needs `ASTER_PROTECTED_OK`); `d496532` + `b6d3373` together restore the largest-drift boundary semantics (or change one line in `tier-sizing.mjs` and regenerate — both encounters are in the artifact); `4c1c58d` addendum (additive, same caveat); `0a8bbac` restores the aggregate file (do not — it was deliberately removed at `1cd5124`); `5ac5cb5` drops the appended sentence and the `maxDriftEncounter` rows (regenerate after); `052d0a2` returns comets to the aphelion sentence; `c6dc7dd` docs (rebuild after any other revert); `3d395fb` founding entry (additive, same caveat).

**Verified:** `npx tsc --noEmit` clean and `node tools/run-tests.mjs` green after every commit — baseline **76 files / 278 tests / 0 failures** at `81252f2`, final **81 / 304 / 0 (1 skipped)** at `052d0a2` (Node v24.18.0, Windows; one run was interrupted by the laptop's ~12-hour sleep — four `v2-render-*` files were killed by the 180 s watchdog on resume, wall clock 42,319 s — and re-run clean; in another run the runner's headline read 302 while the raw per-file summaries summed to 303, see Cleanup C15). Populations recounted from the regenerated artifact at each regeneration: **L0 = 11, L1 = 10,150, L2 = 31,745, structurallyBlindL2 = 688, total = 41,906**; all 41,906 base records byte-identical. Guard ⇔ propagator agreement checked over all 41,906 bodies by propagating them. For all 10,150 L1 bodies the largest-drift date (`maxDriftEncounterCd`) equals `dv-scope-per-body.json` (a different script), and the boundary `encounter` equals an independent re-implementation of the DEC-18-6 criterion over the committed CAD snapshot (the 2,352 count is pinned in the test). Every asset reference in `docs/` (45) resolves; `docs/_staging_v1/` byte-identical by blob id.

**Cut: nothing.** Items 6 and 7 ("attempt") both shipped. The Slice 17 §8 CUT-RULE form was the precedent and was not needed.

**Open items carried forward:**
- L1 boundary semantics — **RATIFIED 2026-09-21** as the earliest material encounter (DEC-18-3's "first materially perturbing encounter"; it can only under-claim). Recorded in the founding doc: the boundary date is model-derived and conservative by construction, not measurement-validated; for 99942 the measured drift stayed near 44k km/yr until the 2029 flyby (`nea-drift-results.json`), so its 2028-09-12 boundary is conservative by roughly seven months. No longer open.
- **Tier artifact load is EAGER — open item.** `src/v2/app/catalog-list/panel.ts` calls `loadTierAssignments()` at module load, so the solar-system page fetches all 10,880,093 bytes of `tier-sizing-per-body.json` on every load before any tier badge renders; the dedicated porkchop page and the compare page fetch it eagerly too (initial `Promise.all`). A lazy or split load (per-body lookup, or a badge-only slice of the artifact) was not natural in Item 5 because every catalog row's badge needs the whole map. Open.
...
- 26 of the 206 structurally-blind comets have Q < 4.95 AU — RESOLVED at `052d0a2`: comets now carry their own sentence ("It is a comet, which Aster's catalog classifies as not safe for two-body Keplerian propagation."); the aphelion sentence is Jupiter-crossing only, and all 482 have Q ≥ 4.95 AU.
...
| Root recursive | `node tools/run-tests.mjs` | **81/81 files pass; 304 tests pass / 0 fail / 1 skipped** (Claude Code, 2026-09-21, Windows / Node v24.18.0, last at `052d0a2` on `claude/s18-close-out`, wall clock 409 s; the 76→81 files and 278→304 tests are the five Slice 18 close-out test files. Prior record: 74/74 · 256 · 0, Hudson, 2026-08-13) |
...
| C17 | **The additive-only hook misses deleted markdown bullets — the common case, not an edge case.** `.githooks/pre-commit:53` counts violations with `git diff --cached -- "$doc" \| grep -c '^-[^-]'`. A deleted bullet line (`- item`) renders in the diff as `-- item`, whose second character is also `-`, so the pattern does not match and the deletion passes unblocked. `INVARIANTS.md` and the `SLICE_*_FOUNDING.md` files are largely bullet lists, so most of what the rule protects is exactly what the pattern cannot see. Demonstrated 2026-09-22 against commit `2032b9e`, which deletes one bullet from `mcp/README.md`: `--stat` reports `1 deletion(-)`, the hook pattern reports `0`, and `grep '^-' \| grep -v '^--- a/'` reports `1`. Fix is the latter form; `.githooks/` is a protected path, so it needs `ASTER_PROTECTED_OK` plus a dispatch. Until then, every additive-only proof written into a report must use the corrected pattern — all appends in the 2026-09-22 docs-correction pass were re-verified with it and showed 0 real deletions. |
