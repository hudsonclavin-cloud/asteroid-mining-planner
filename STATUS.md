...
| Local HEAD (branch `claude/s18-close-out`) | `4c1c58d` | Slice 18 close-out run: eight code commits `d8c4681` … `4d3bae2`, the founding-doc close record `4d3267c`, a first STATUS close `63841cc`, then the self-caught L1-boundary correction `d496532` + docs `b6d3373` + founding-doc addendum `4c1c58d`; this STATUS commit lands on top of it |

**Measured pin lag (2026-09-21):** before this edit, `git rev-list --count 4c1c58d..HEAD` measured 0 commits and `git rev-list --count 81252f2..HEAD` measured 13. After this STATUS commit lands, those measure 1 and 14. STATUS commits LAST in any sequence.
**Push state:** origin/main is at `81252f2`. Branch `claude/s18-close-out` holds the close-out run's fourteen local-only commits and is NOT merged. No agent pushes, ever. The merge is Hudson's: `git checkout main; git merge claude/s18-close-out --ff-only; git hpush`.
...
**Deploy boundary:** live `docs/` is `81252f2` (rebuilt 2026-09-20; live bundles `porkchopV2-Ci6r-fmN.js`, `solarSystemV2-DFJ0f0Yh.js`, `compareV2-CEg78aFw.js`). The close-out rebuild is `b6d3373` on the branch (bundles built at `4d3bae2`, tier artifact refreshed at `b6d3373` after the L1-boundary correction) — live only once merged and pushed — with entry bundles `porkchopV2-DPwWKoPv.js`, `solarSystemV2-BM6ai_je.js`, `compareV2-Br2FOkFz.js`, shared chunks `porkchop-view-CwA2OpZb.js`, `tier-assignments-Dlnkrv_W.js`, `resolve-catalog-body-pmopDgKo.js`, `store-CkJiJhKO.js`, and the tier artifact `docs/tier-sizing-per-body.json` (11.01 MB, sha256 `a2c02234…`, identical to the `tools/` copy). The `5a00907` boundary previously recorded here was stale: `81252f2` had already rebuilt `docs/` on 2026-09-20.
...
**Verified:** `npx tsc --noEmit` clean and `node tools/run-tests.mjs` green after every commit — baseline **76 files / 278 tests / 0 failures** at `81252f2`, final **81 / 303 / 0** at `d496532` (Node v24.18.0, Windows). Populations recounted from the regenerated artifact at each regeneration: **L0 = 11, L1 = 10,150, L2 = 31,745, structurallyBlindL2 = 688, total = 41,906**; all 41,906 base records byte-identical. Guard ⇔ propagator agreement checked over all 41,906 bodies by propagating them. For all 10,150 L1 bodies the largest-drift date (`maxDriftEncounterCd`) equals `dv-scope-per-body.json` (a different script), and the boundary `encounter` equals an independent re-implementation of the DEC-18-6 criterion over the committed CAD snapshot (the 2,352 count is pinned in the test). Every asset reference in `docs/` (45) resolves; `docs/_staging_v1/` byte-identical by blob id.

**Cut: nothing.** Items 6 and 7 ("attempt") both shipped. The Slice 17 §8 CUT-RULE form was the precedent and was not needed.

**Open items carried forward:**
- **Ratify the L1 boundary semantics** (`d496532`): the earliest individually-material encounter, per DEC-18-7's "materially perturbing encounter" and the Front C design — versus the largest-drift encounter dv-scope records. Conservative and reversible in one line; the visible consequence is Apophis's sentence (2028-09-12 / 2.6 m/s instead of 2029-04-13 / 2,776 m/s).
- Eager ~11 MB tier-artifact fetch on the solar-system page (module load in `src/v2/app/catalog-list/panel.ts`) and now on the dedicated porkchop page. A lazy load was not natural in Item 5 — every catalog row's badge needs it. Open.
...
| Root recursive | `node tools/run-tests.mjs` | **81/81 files pass; 303 tests pass / 0 fail** (Claude Code, 2026-09-21, Windows / Node v24.18.0, last at `d496532` on `claude/s18-close-out`; the 76→81 files and 278→303 tests are the five Slice 18 close-out test files. Prior record: 74/74 · 256 · 0, Hudson, 2026-08-13) |
